...
def compute_keyword_overlap(doc1_tokens, doc2_tokens, tfidf_matrix=None, idx1=None, idx2=None):
    """
    Compute semantic similarity between two documents.
    
    If tfidf_matrix is provided, uses TF-IDF cosine similarity (more meaningful).
    Otherwise falls back to Jaccard token overlap (legacy).
    """
    # Prefer TF-IDF if available (captures semantic meaning, not just keyword overlap)
...
def compute_global_embedding_health(df, coordinates, n_neighbors=5, tfidf_matrix=None):
    """
    Compute average semantic similarity among nearest neighbors in embedding space.
    Returns health score (0-1) where 1 = perfect alignment, 0 = no alignment.
    
    Uses TF-IDF cosine similarity (if available) for more meaningful scoring,
    falls back to Jaccard token overlap if TF-IDF unavailable.
    """
    ...
    try:
        ...
        for i in range(len(df)):
            ...
            # Check similarity with neighbors (excluding self at index 0)
            for neighbor_idx in indices[i][1:]:
                neighbor_tokens = df.iloc[neighbor_idx].get('__tokens', [])
                # Use TF-IDF if available (more semantically meaningful)
        ...
        if similarities:
            health_score = float(np.mean(similarities))
            return health_score
        return None
    except Exception:
        return None
...
@st.cache_data
def _compute_pca_fallback_tsne(base_emb):
    """Cache PCA fallback to avoid recomputing on every rerun."""
    st.warning("⏳ Computing t-SNE with PCA fallback (this happens on every app rerun if t-SNE file is unavailable)")
    return PCA(n_components=2).fit_transform(base_emb)

@st.cache_data
def _compute_pca_fallback_umap(base_emb):
    """Cache PCA fallback to avoid recomputing on every rerun."""
    st.warning("⏳ Computing UMAP with PCA fallback (this happens on every app rerun if UMAP file is unavailable)")
    return PCA(n_components=2).fit_transform(base_emb + 0.01)

if coords_tsne is None:
    coords_tsne = _compute_pca_fallback_tsne(base_embeddings)
if coords_umap is None:
    coords_umap = _compute_pca_fallback_umap(base_embeddings + 0.01)
...
def _compute_embedding_health():
    ...
    # Pass TF-IDF matrix for semantic similarity scoring (much more meaningful than token overlap)
    health = compute_global_embedding_health(df, coords_tsne, n_neighbors=5, tfidf_matrix=tfidf_matrix)
    return health
