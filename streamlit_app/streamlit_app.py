...
processed_csv_path = find_file(bundle_candidates('full_dataset_with_new_id.csv') + bundle_candidates('processed_data_with_clusters.csv'))

# Consistency fix: determine if we should enforce a specific root based on the CSV found
forced_root = None
if processed_csv_path and 'full_dataset_with_new_id.csv' in processed_csv_path:
...
def make_plot(df_plot, xcol, ycol, selected_ids, search_ids, title, hover_cols, color_mode='Selection', focused_ids=None):
    ...
    # In cluster mode, add focused points as an overlay scatter for visibility
    if color_mode == 'Cluster' and focused_ids:
        foc_df = df_plot[df_plot['doc_id'].isin(focused_ids)]
        if not foc_df.empty:
            ...
            fig.add_trace(go.Scatter(
                ...
                name='Focused',
                customdata=foc_df[['doc_id']].values,
                hoverinfo='text',
                hovertext=foc_df['doc_id'].tolist()
            ))
...
with st.sidebar:
    ...
    with st.expander('Dataset & Display', expanded=True):
        ...
        detailed_hover = st.checkbox('Show detailed hover info', value=True, help='If checked, hovering over a point shows rich metadata. Uncheck for a cleaner, faster view with just IDs.')
...
if not detailed_hover:
    ...
    hover_cols = ['doc_id']
    if 'Slogan' in df_work.columns:
        hover_cols.append('Slogan')
    if 'cluster' in df_work.columns:
        hover_cols.append('cluster')
    if 'Dashboard Type' in df_work.columns:
        hover_cols.append('Dashboard Type')
...
with tab_explore:
    ...
    if view_mode == "Single (Focus)":
        ...
        with pcol1:
            ...
            # Update opacity/size if customizable
            fig_focus.update_traces(marker=dict(opacity=point_alpha, size=8 * point_size_scale))
        ...
        # Apply visual tweaks
        fig_tsne.update_traces(marker=dict(opacity=point_alpha))
        for t in fig_tsne.data: t.marker.size = t.marker.size * point_size_scale
        ...
        # Apply visual tweaks
        fig_umap.update_traces(marker=dict(opacity=point_alpha))
        for t in fig_umap.data: t.marker.size = t.marker.size * point_size_scale
        ...
        # Apply visual tweaks
        fig_pca.update_traces(marker=dict(opacity=point_alpha))
        for t in fig_pca.data: t.marker.size = t.marker.size * point_size_scale
...
# ── Analysis Section ──
# Use tabs to organize analysis views instead of a long vertical scroll
tab_docs, tab_stats, tab_keywords = st.tabs(["Selected Documents", "Cluster Stats", "Top Keywords"])
...
with tab_keywords:
    if st.session_state['selected_ids']:
        ...
        if all_toks:
            ...
            counts = Counter(all_toks).most_common(30)
